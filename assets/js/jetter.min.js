...
(function() {
	window.J = {
			...
			if (a.has("#") || a.has(".") || a.has("[")) {
				...
				for (var i = 1; i < c.length; i++) {
					if (cnum > 0) {
						...
						var h = g.indexOf("=");
						e.attr(g.substring(0, h), g.substring(h + 1))
					}
				}
				return e
			} else {
		...
		jsonp: function(b) {
			if (!b.url) {
				...
				var c = ('_jsonp' + Math.random()).replace(".", "").substring(0, 15);
				var d = J.tag("head");
				b.data[_checkArg(b.callback, "callback")] = c;
				var e = J.cre('script');
				d.append(e);
				window[c] = function(a) {
					d.removeChild(e);
				...
				if (b.dataType != undefined && b.dataType.toUpperCase() == "JSON") {
					e.attr("src", b.url + '?json=' + encodeURIComponent(JSON.stringify(b.data)))
				} else {
					e.attr("src", b.url + '?' + _formatParams(b.data))
				}
				b.time = _checkArg(b.time, 5000);
				e.timer = setTimeout(function() {
					window[c] = null;
					d.removeChild(e);
					b.timeout && b.timeout({
						message: ((!b.message) ? "timeout" : b.message)
					})
				}, b.time)
			}
		},
		...
		input: function(c, d, e) {
			...
			if (c.constructor == String) {
				f.append(J.cre("div").txt(_checkArg(c, (this.language == "CHINESE") ? "信息输入" : "Input Information")));
				_appendOneInput((this.language == "CHINESE") ? "请输入：" : "Please input:", null, null, null)
			} else if (c.constructor == Array) {
				f.append(J.cre("div").txt(_checkArg(c[0], (this.language == "CHINESE") ? "信息输入" : "Input Information")));
				_appendOneInput(c[1], c[2], c[3], c[4]);
				if (c[3] != undefined) {
					this.initValid(f)
				}
			} else {
				f.append(J.cre("div").txt(_checkArg(c.title, (this.language == "CHINESE") ? "信息输入" : "Input Information")));
	...
	HTMLElement.prototype.allHtml = function(a) {
		if (a == undefined) {
			return J.cre("div").append(this.clone()).html()
		} else {
			var b = this.index();
			var c = this.parent().append(a, b);
			this.remove();
			return c.child(b)
		}
	};
	...
	HTMLElement.prototype.tip = function(a) {
		if (J.id("jetTip") == undefined) {
			J.body().append(J.cre("span#jetTip"))
		}
	...
	HTMLCollection.prototype.tip = NodeList.prototype.tip = function(b) {
	...
	function _checkSlideHeight(e, a, b, c, d) {
		if (e.style.height == "" || e.style.height == "auto") {
			if (d && e.css("display") == "none") {
				e.addClass("j-none")
			}
			e.css("height", e.css("height"));
			e.removeClass("j-none");
			e.attr("j-h-auto", "true");
			setTimeout(function() {
				_animateBase(e, "j-slide", a, b, c, d)
			}, 50)
		} else {
			_animateBase(e, "j-slide", a, b, c, d)
		}
	...
	function _attachEvent(a, b, c) {
		if (document.addEventListener) {
			a.addEventListener(b.substring(2), _checkFunction(c), false)
		} else if (document.attachEvent) {
			a.attachEvent(b, _checkFunction(c))
		}
	}
	...
	function _inputOk() {
		if (m != null) {
			J.id("jetInputContent").validate(function() {
				m(J.id("jetInputContent").findClass("jet-input").val());
				m = null;
				_inputClose()
			})
		} else {
	...
	function _appendOneInput(a, b, c, p) {
		...
		J.id("jetInputContent").append(J.cre("div").addClass("jet-input-text").txt(a));
		var d = J.cre("input.jet-input[type=text]");
		if (b != undefined) {
			d.val(b)
		}
		if (c != undefined) {
	...
	function _addNoteWrapper() {
		var a = J.cre("span").addClass("jet-icon-circle").append([J.cre("span"), J.cre("span")]);
		var b = J.cre("div").attr("id", "jetNoteIcon").addClass("jet-icon-wrapper").append(a);
		var c = J.cre("div").attr("id", "jetNoteContent");
		var d = J.cre("div").attr({
			"id": "jetNoteLittleWrapper",
			"jet-style": J.noteStyle
		}).append([b, c]);
		J.body().append(J.cre("div").attr({
			"id": "jetNoteWrapper",
			"onclick": "J.close()"
		}).append(d))
	};
	...
	function _addConfirmWrapper() {
		var a = J.cre("span").addClass("jet-icon-circle jet-no-border").append([J.cre("span").addClass("jet-icon-part-ok1 jet-rotate-45"), J.cre("span").addClass("jet-icon-part-ok2 jet-rotate-45")]);
		var b = J.cre("span").addClass("jet-icon-circle jet-no-border").append([J.cre("span").addClass("jet-icon-part-x jet-rotate-45"), J.cre("span").addClass("jet-icon-part-x jet-rotate-045")]);
		var c = J.cre("div").attr({
			"id": "jetConfirmOk",
			"onclick": "J.confirmOk()"
		}).append(a);
		var d = J.cre("div").attr({
		...
		var e = J.cre("div").attr("id", "jetConfirmBtnWrapper").append([c, d]);
		var f = J.cre("div").attr("id", "jetConfirmContent");
		var g = J.cre("div").attr("id", "jetConfirmLittleWrapper").append([f, e]);
		var h = J.cre("div").attr("id", "jetConfirmWrapper").append(g);
		J.body().append(h)
	};
	...
	function _addInputWrapper() {
		...
		var e = J.cre("div").attr("id", "jetInputBtnWrapper").append([c, d]);
		var f = J.cre("div").attr("id", "jetInputContent");
		var g = J.cre("div").attr("id", "jetInputLittleWrapper").append([f, e]);
		var h = J.cre("div").attr("id", "jetInputWrapper").append(g);
		J.body().append(h)
	}
})();
